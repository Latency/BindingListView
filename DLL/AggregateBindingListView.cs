/****************************************************************************
 * File:      AggregateBindingListView.cs
...
namespace Equin.ApplicationFramework {
  public class AggregateBindingListView<T> : Component, IBindingListView, IRaiseItemChangedEvents, ICancelAddNew, ITypedList, IEnumerable<T> {
    ...
    /// </summary>
    /// <exception cref="ArgumentNullException" accessor="set"><paramref name="source" /> or <paramref name="predicate" /> is null.</exception>
    /// <exception cref="InvalidCastException" accessor="set">An element in the sequence cannot be cast to type <paramref name="TResult" />.</exception>
    /// <exception cref="InvalidSourceListException" accessor="set">Condition. </exception>
    /// <exception cref="FormatException" accessor="set">The format item in <paramref name="format" /> is invalid.-or- The index of a format item is not zero. </exception>
    [Browsable(false)]
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
    public IList SourceLists {
      get { return _sourceLists; }
      set {
        if (value == null) throw new ArgumentNullException("SourceLists", Resources.SourceListsNull);

        // Check that every item in each list is of type T.
        foreach (var obj in value) {
          if (obj == null) throw new InvalidSourceListException();

          IList list = null;
          if (!string.IsNullOrEmpty(DataMember)) {
            foreach (PropertyDescriptor pd in TypeDescriptor
              .GetProperties(obj)
              .Cast<PropertyDescriptor>()
              .Where(pd => pd.Name == DataMember)) {
              list = pd.GetValue(obj) as IList;
              break;
            }
          } else if (obj is IListSource) {
            var src = obj as IListSource;
            if (src.ContainsListCollection) list = src.GetList()[0] as IList;
            else list = (obj as IListSource).GetList();
          } else if (!(obj is ICollection<T>)) list = obj as IList;
          else {
            // We have a typed collection, so can skip the item-by-item check.
            continue;
          }

          if (list == null) throw new InvalidSourceListException();

          if (list.Cast<object>().Any(item => !(item is T))) {
            throw new InvalidSourceListException(string.Format(Resources.InvalidListItemType, typeof (T).FullName));
          }
        }

        var bindingList = _sourceLists as IBindingList;

        // Un-hook old list changed event.
        if (bindingList != null && bindingList.SupportsChangeNotification) bindingList.ListChanged -= SourceListsChanged;

        foreach (var bl in _sourceLists
          .Cast<object>()
          .OfType<IBindingList>()
          .Where(bl => bl.SupportsChangeNotification)) {
    ...
    public ObjectView<T> this[int index] { get { return _sourceIndices[index].Key.Item; } }
    ...
    private bool ShouldSerializeListMember() { return !string.IsNullOrEmpty(DataMember); }
    ...
    public void SuspendAutoFilterAndSort() { _autoFilterAndSortSuspended = true; }
    public void ResumeAutoFilterAndSort() { _autoFilterAndSortSuspended = false; }
    ...
    protected void FilterAndSort() {
      ...
      foreach (var sourceList in GetSourceLists()) {
        for (var i = 0; i < sourceList.Count; i++) {
          var item = (T) sourceList[i];
          ObjectView<T> editableObject;
          if (_filter.Include(item)) {
            if (_objectViewCache.ContainsKey(item)) editableObject = _objectViewCache[item];
            else {
      ...
      if (_comparer != null) newList.Sort(_comparer);

      // Now we can append any new items to the end of the view.
      newList.AddRange(_sourceIndices.Where(kvp => kvp.Value == -1));
    ...
    protected virtual void SourceListsChanged(object sender, ListChangedEventArgs e) {
      if (e.ListChangedType == ListChangedType.ItemAdded) {
        var list = SourceLists[e.NewIndex] as IList;
        if (list == null) {
        ...
        if (list is IBindingList) {
          ...
          (list as IBindingList).ListChanged += SourceListChanged;
        }
        ...
        var list = _savedSourceLists[e.NewIndex];
        if (list != null) {
          if (list is IBindingList) (list as IBindingList).ListChanged -= SourceListChanged;
    ...
    private void SourceListChanged(object sender, ListChangedEventArgs e) {
      if (_autoFilterAndSortSuspended) return;
      ...
      switch (e.ListChangedType) {
        case ListChangedType.ItemAdded:
          ...
          if (newIndex > -1) {
            ...
            for (var i = newIndex + 1; i < Count; i++) OnListChanged(ListChangedType.ItemMoved, i - 1, i);
          } else {
        ...
        case ListChangedType.ItemChanged:
          ...
          var oldIndex = _sourceIndices.IndexOfSourceIndex(sourceList, e.NewIndex);
          ...
          } else OnListChanged(ListChangedType.ItemDeleted, oldIndex);
          break;
        ...
        case ListChangedType.ItemDeleted:
          ...
          for (var i = 0; i < _sourceIndices.Count; i++) if (_sourceIndices[i].Value > e.NewIndex) _sourceIndices[i] = new KeyValuePair<ListItemPair<T>, int>(_sourceIndices[i].Key, _sourceIndices[i].Value - 1);
    ...
    public AggregateBindingListView(IContainer container)
      ...
      var o = Site as ISynchronizeInvoke;
      if (o != null) SynchronizingObject = o;
    }
    ...
    /// </summary>
    private readonly List<IList> _savedSourceLists;
    ...
    ///   in FilterAndSort().
    /// </summary>
    private readonly Dictionary<T, ObjectView<T>> _objectViewCache;
    ...
    ///   No new object provided by the AddingNew event handler and
    ///   <typeparamref name="T" /> has no default public constructor.
    /// </exception>
    ...
    protected virtual T OnAddingNew() {
      ...
      if (AddingNew != null) {
        ...
        var o = args.NewObject is T ? (T) args.NewObject : default(T);
        if (o != null) return o;
      }
      ...
      throw new InvalidOperationException(Resources.CannotAddNewItem);
    }
    ...
    public void EndNew(int itemIndex) {
      ...
      if (itemIndex > -1 && itemIndex < _sourceIndices.Count &&
        ...
        if (!(_newItemsList is IBindingList) || !((IBindingList) _newItemsList).SupportsChangeNotification) {
          if (!_autoFilterAndSortSuspended) FilterAndSort();
          OnListChanged(ListChangedType.Reset, -1);
        }
    ...
    public IList NewItemsList {
      get { return _newItemsList; }
      set {
        if (value != null && !_sourceLists.Contains(value))
          throw new ArgumentException(Resources.SourceListNotFound);
        _newItemsList = value;
      }
    }
    ...
    protected virtual void EndedItemEdit(object sender, EventArgs e) {
      ...
      var editableObject = (ObjectView<T>) sender;

      // Check if filtering removed the item from view
      // by getting the index before and after
      var oldIndex = _sourceIndices.IndexOfItem(editableObject.Object);
      FilterAndSort();
      var newIndex = _sourceIndices.IndexOfItem(editableObject.Object);
      // if item was filtered out then the newIndex == -1
      if (newIndex > -1) {
        if (oldIndex == newIndex) OnListChanged(ListChangedType.ItemChanged, newIndex);
        else OnListChanged(ListChangedType.ItemMoved, newIndex, oldIndex);
      } else OnListChanged(ListChangedType.ItemDeleted, oldIndex);
    }
    ...
    protected virtual void OnListChanged(ListChangedEventArgs e) {
      if (ListChanged != null) {
        ...
        if (SynchronizingObject != null && SynchronizingObject.InvokeRequired) {
          SynchronizingObject.Invoke(ListChanged, new object[] {
            this,
            e
          });
        } else
    ...
    private void OnListChanged(ListChangedType listChangedType, int newIndex) { OnListChanged(new ListChangedEventArgs(listChangedType, newIndex)); }
    ...
    private void OnListChanged(ListChangedType listChangedType, int newIndex, int oldIndex) { OnListChanged(new ListChangedEventArgs(listChangedType, newIndex, oldIndex)); }

    #endregion

    #region Filtering

    public void ApplyFilter(IItemFilter<T> filter) { Filter = filter; }

    public void ApplyFilter(Predicate<T> includeItem) {
      if (includeItem == null) throw new ArgumentNullException("includeItem", Resources.IncludeDelegateCannotBeNull);
    ...
    private bool ShouldSerializeFilter() { return (Filter != IncludeAllItemFilter<T>.Instance); }
    ...
    public void ApplySort(ListSortDescriptionCollection sorts) { SortDescriptions = sorts; }
    ...
    /// </param>
    /// <example>
    ///   <code>view.ApplySort("Surname, FirstName, Age DESC");</code>
    /// </example>
    public void ApplySort(string sort) {
      ...
      var sorts = sort.Split(SortDelimiter);
      ...
      for (var i = 0; i < sorts.Length; i++) {
        ...
        if (pos == -1) {
          ...
          if (string.Compare(dir, SortDescendingModifier, true, CultureInfo.InvariantCulture) == 0) direction = ListSortDirection.Descending;
          else {
      ...
      get {
        if (IsSorted) {
          ...
          foreach (ListSortDescription sort in SortDescriptions) {
            ...
            if (sort.SortDirection == ListSortDirection.Descending) sb.Append(' ').Append(SortDescendingModifier);
    ...
    private bool ShouldSerializeSort() { return !String.IsNullOrEmpty(Sort); }
    ...
    public ListSortDirection SortDirection {
      get {
        if (IsSorted) return SortDescriptions[0].SortDirection;
    ...
    public PropertyDescriptor SortProperty {
      get {
        if (IsSorted) return SortDescriptions[0].PropertyDescriptor;
        // We don't really want to throw exceptions.
        // Calling code should have checked IsSorted to know the true situation.
        return null;
      }
    ...
    private class SortComparer : IComparer<KeyValuePair<ListItemPair<T>, int>> {
      private readonly Dictionary<ListSortDescription, Comparison<T>> _comparisons;
      ...
      public SortComparer(ListSortDescriptionCollection sorts) {
        ...
        foreach (ListSortDescription sort in sorts) _comparisons[sort] = BuildComparison(sort.PropertyDescriptor.Name, sort.SortDirection);
      }
      ...
      public int Compare(KeyValuePair<ListItemPair<T>, int> x, KeyValuePair<ListItemPair<T>, int> y) {
        return (from ListSortDescription sort in _sorts
                select _comparisons[sort](x.Key.Item.Object, y.Key.Item.Object)).FirstOrDefault(result => result != 0);
      }
      ...
      private static Comparison<T> BuildComparison(string propertyName, ListSortDirection direction) {
        ...
        if (typeof (IComparable).IsAssignableFrom(pi.PropertyType)) {
          if (pi.PropertyType.IsValueType) return BuildValueTypeComparison(pi, direction);
          var getProperty = BuildGetPropertyMethod(pi);
          return delegate(T x, T y) {
            int result;
            var value1 = getProperty(x);
            var value2 = getProperty(y);
            if (value1 != null && value2 != null) result = (value1 as IComparable).CompareTo(value2);
            else if (value1 == null && value2 != null) result = -1;
            else if (value1 != null && value2 == null) result = 1;
            else result = 0;

            if (direction == ListSortDirection.Descending) result *= -1;
            return result;
          };
        }
        if (pi.PropertyType.IsGenericType && pi.PropertyType.GetGenericTypeDefinition() == typeof (Nullable<>)) return BuildNullableComparison(pi, direction);
        return (o1, o2) => o1.Equals(o2) ? 0 : o1.ToString().CompareTo(o2.ToString());
      }
      ...
      private static GetPropertyDelegate BuildGetPropertyMethod(PropertyInfo pi) {
        ...
        var dm = new DynamicMethod("__blw_get_" + pi.Name, typeof (object), new[] {
          typeof (T)
        }, typeof (T), true);
      ...
      private static Comparison<T> BuildRefTypeComparison(PropertyInfo pi, ListSortDirection direction) {
        ...
        var dm = new DynamicMethod("Get" + pi.Name, typeof (int), new[] {
          typeof (T),
          typeof (T)
        }, typeof (T), true);
        var il = dm.GetILGenerator();
    ...
    private class ExternalSortComparer<U> : IComparer<KeyValuePair<ListItemPair<U>, int>> {
      ...
      public ExternalSortComparer(IComparer<U> comparer) { _comparer = comparer; }
      public int Compare(KeyValuePair<ListItemPair<U>, int> x, KeyValuePair<ListItemPair<U>, int> y) { return _comparer.Compare(x.Key.Item.Object, y.Key.Item.Object); }
    }

    private class ExternalSortComparison<U> : IComparer<KeyValuePair<ListItemPair<U>, int>> {
      private readonly Comparison<U> _comparison;
      public ExternalSortComparison(Comparison<U> comparison) { _comparison = comparison; }
      public int Compare(KeyValuePair<ListItemPair<U>, int> x, KeyValuePair<ListItemPair<U>, int> y) { return _comparison(x.Key.Item.Object, y.Key.Item.Object); }
    }
    ...
    public int Find(PropertyDescriptor property, object key) {
      for (var i = 0; i < _sourceIndices.Count; i++) if (property.GetValue(_sourceIndices[i].Key.Item.Object).Equals(key)) return i;
      return -1;
    }
    ...
    public int Find(string propertyName, object key) {
      ...
      if (pd != null) return Find(pd, key);
      throw new ArgumentException(string.Format(Resources.PropertyNotFound, propertyName, typeof (T).FullName), "propertyName");
    }
    ...
    bool IBindingList.AllowEdit {
      get { return SourceLists.OfType<IBindingList>().All(list => list.AllowEdit); }
    }
    ...
    bool IBindingList.AllowRemove {
      get { return SourceLists.OfType<IBindingList>().All(list => list.AllowRemove); }
    }
    ...
    int IList.Add(object value) {
      if (value != null)
        throw new NotSupportedException(Resources.CannotAddItem);
      AddNew();
      return Count - 1;
    }
    ...
    ///   Cannot clear this view.
    /// </exception>
    void IList.Clear() { throw new NotSupportedException(Resources.CannotClearView); }
    ...
    bool IList.Contains(object item) {
      ...
      var key = item as ObjectView<T>;
      if (key != null) return _sourceIndices.ContainsKey(key);
      var item1 = item is T ? (T) item : default(T);
      return item1 != null && _sourceIndices.ContainsItem(item1);
    }
    ...
    int IList.IndexOf(object item) {
      var view = item as ObjectView<T>;
      if (view != null) return _sourceIndices.IndexOfKey(view);
      if (item is T) return _sourceIndices.IndexOfItem((T) item);
      return -1;
    }
    ...
    ///   Cannot insert an external item into this collection.
    /// </exception>
    void IList.Insert(int index, object value) { throw new NotSupportedException(Resources.CannotInsertItem); }
    ...
    bool IList.IsReadOnly {
      get {
        foreach (var list in SourceLists) {
          if (list is IBindingList) {
            if (!(list as IBindingList).IsReadOnly) return false;
          } else return false;
        }
        return true;
      }
    ...
    void IList.RemoveAt(int index) {
      ...
      if (sourceIndex > -1) {
        ...
        if (!(sourceList is IBindingList) || !((IBindingList) sourceList).SupportsChangeNotification) {
          FilterAndSort();
          OnListChanged(ListChangedType.Reset, -1);
        }
      } else {
    ...
    object IList.this[int index] {
      ...
      set {
        ...
        // in this manner.
        throw new NotSupportedException(Resources.CannotSetItem);
      }
    }
    ...
    /// <summary>
    ///   Copies the <see cref="ObjectView&lt;T&gt;" /> objects of the view to an <see cref="System.Array" />, starting at a
    ...
    ///   view. The System.Array must have zero-based indexing.
    /// </param>
    /// <param name="index">The zero-based index in array at which copying begins. </param>
    void ICollection.CopyTo(Array array, int index) { _sourceIndices.Keys.CopyTo(array, index); }

    /// <summary>
    ///   Gets a value indicating whether access to the <see cref="System.Collections.ICollection" /> is synchronized (thread
    ///   safe).
    /// </summary>
    ...
    ///   Not supported.
    /// </summary>
    object ICollection.SyncRoot { get { throw new NotSupportedException(Resources.SyncAccessNotSupported); } }
    ...
    /// </summary>
    [Browsable(false)]
    public int Count { get { return _sourceIndices.Count; } }
    ...
    IEnumerator<T> IEnumerable<T>.GetEnumerator() {
      return _sourceIndices.Select(t => t.Key.Item.Object).GetEnumerator();
    }
    ...
    IEnumerator IEnumerable.GetEnumerator() { return _sourceIndices.GetKeyEnumerator(); }
    ...
    /// <summary>
    ///   Returns the <see cref="System.ComponentModel.PropertyDescriptorCollection" /> that represents the properties on each
    ///   item used to bind data.
    /// </summary>
    /// <param name="listAccessors">
    ///   Array of property descriptors to navigate object hirerachy to actual item object. It can be
    ///   null.
    /// </param>
    /// <returns>
    ///   The System.ComponentModel.PropertyDescriptorCollection that represents the properties on each item used to
    ///   bind data.
    /// </returns>
    PropertyDescriptorCollection ITypedList.GetItemProperties(PropertyDescriptor[] listAccessors) {
      PropertyDescriptorCollection originalProps;

      var lists = GetSourceLists().GetEnumerator();
      ...
      if (listAccessors != null && listAccessors.Length > 0) {
        ...
        if (type.IsGenericType && type.GetGenericTypeDefinition() == typeof (ObjectView<>)) originalProps = originalProps[0].GetChildProperties();
      }

      return new PropertyDescriptorCollection(AddProvidedViews(originalProps).ToArray());
    }

    protected internal bool ShouldProvideView(PropertyDescriptor property) { return ProvidedViewPropertyDescriptor.CanProvideViewOf(property); }

    protected internal string GetProvidedViewName(PropertyDescriptor sourceListProperty) { return sourceListProperty.Name + "View"; }
    ...
    private static Type GetProvidedViewType(PropertyDescriptor sourceListProperty) {
      ...
      Type typeParam = null;
      foreach (var interfaceType in sourceListProperty.PropertyType.GetInterfaces().Where(interfaceType => interfaceType.IsGenericType && interfaceType.GetGenericTypeDefinition() == typeof (IList<>)))
      ...
      var viewTypeDef = typeof (BindingListView<>);
    ...
    internal IEnumerable<PropertyDescriptor> AddProvidedViews(PropertyDescriptorCollection properties) {
      ...
      foreach (PropertyDescriptor prop in properties) {
        if (ShouldProvideView(prop)) {
          var name = GetProvidedViewName(prop);
    ...
    string ITypedList.GetListName(PropertyDescriptor[] listAccessors) { return GetType().Name; }
    ...
    public ListSortDescription CreateListSortDescription(string propertyName, ListSortDirection direction) {
      var pd = GetPropertyDescriptor(propertyName);
      if (pd == null) throw new ArgumentException(string.Format(Resources.PropertyNotFound, propertyName, typeof (T).FullName), "propertyName");
    ...
    private static PropertyDescriptor GetPropertyDescriptor(string propertyName) { return TypeDescriptor.GetProperties(typeof (T)).Find(propertyName, false); }
    ...
    /// </summary>
    /// <param name="objectView">The <see cref="ObjectView&lt;T&gt;" /> to listen to.</param>
    private void HookPropertyChangedEvent(ObjectView<T> editableObject) { editableObject.PropertyChanged += ItemPropertyChanged; }

    /// <summary>
    ///   Detaches the event handler from the <see cref="ObjectView&lt;T&gt;" />'s PropertyChanged event.
    /// </summary>
    /// <param name="objectView">The <see cref="ObjectView&lt;T&gt;" /> to stop listening to.</param>
    private void UnHookPropertyChangedEvent(ObjectView<T> editableObject) { editableObject.PropertyChanged -= ItemPropertyChanged; }

    private void BuildSavedList() {
      _savedSourceLists.Clear();
      foreach (IList list in GetSourceLists()) _savedSourceLists.Add(list);
    }
    ...
    /// <exception cref="NotSupportedException"><paramref name="component" /> is a cross-process remoted object.</exception>
    protected IEnumerable<IList> GetSourceLists() {
      foreach (var obj in _sourceLists) {
        if (!string.IsNullOrEmpty(DataMember)) {
          var found = false;
          ...
          if (src.ContainsListCollection) {
            var list = src.GetList();
            if (list != null && list.Count > 0) {
              list = list[0] as IList;
              yield return list;
            } else yield return null;
          } else yield return src.GetList();
        } else yield return obj as IList;
      }
    }
